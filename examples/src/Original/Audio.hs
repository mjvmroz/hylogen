...
dashiki = 0.01 *^ vec4 (r, g, b, 1) + 1.1 *^ bb
 where
  ...
  tim = time / 10e1
  v =
    len (uvN - mouse + vec2 (sin tim, cos tim)) * y_ mouse
      & \x ->
        10 * sin x
          & tan
  bb = texture2D backBuffer (0.5 * fn uvN + 0.5)
...
satanic = 0.01 *^ vec4 (r, g, b, 1) + 1.1 *^ bb
 where
  ...
  fn n =
    y_ (uvN - mouse + vec2 (sin tim, cos tim))
...
candyRoad2 = 0.01 *^ vec4 (r v, g v, b v, 1) + 0.99 *^ bb
 where
  ...
  v =
    (x_ audio & linexp (0, 1, 1, 100)) - len (uvN - vec2 (0.5 * y_ audio, -0.333)) * y_ audio
...
candyRoad4 =
  0.01 *^ vec4 (r v, g v, b v, 1) + 0.99 *^ (bb)
    & (clamp 0 1)
 where
  r :: Vec1 -> Vec1
...
rclogo2 = logo
 where
  ...
  chan1OrWhite =
    sel
      (f uvN `lt` 1 * f uvN `gt` (-1))
      (mix (z_ audio + 0.5) (texture2D channel1 (f uvN)) bb)
      bb
  f =
    id
      -- >>> (\x -> x - mouse)
 ...
 where
  ...
  chan1OrWhite =
    sel
      ...
      ((\x -> vec4 (vec3 (z_ x, x_ x, y_ x), w_ x)) bb)
...
rclogo16 = logo
 where
  ...
  chan1OrWhite =
    sel
      ((ff . f) uvN `lt` 1 * (ff . f) uvN `gt` (-1))
      (rotColor $ mix (w_ stuff) stuff bb)
      (rotColor . fade $ bb)

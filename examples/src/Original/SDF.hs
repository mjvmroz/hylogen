...
raymarch0 =
  id
    ...
    . (\x -> vec4 (x, 1))
    . fst
    $ foldr (fn . fromInteger) (black, 0) [0 .. 10]
  where
    eye = vec3 (0, 0, -1)
    up = vec3 (0, 1, 0)
    right = vec3 (1, 0, 0)
...
raymarch1 =
  id
    ...
    . (\x -> vec4 (x, 1))
    . (\(x, _, _) -> x)
    $ foldr (fn . fromInteger) (black, 0, true) [1 .. maxSteps]
  where
    eye = vec3 (0, 0, -1 * (mouse & y_))
    up = vec3 (0, 1, 0)
    right = vec3 (1, 0, 0)
...
raymarch7 =
  id
    ...
    . (\x -> vec4 (x, x & z_))
    . (\(x, _, _) -> x)
    . foldr (fn . fromInteger) (black, 0, true)
    $ [1 .. maxSteps]
  where
    ...
    fn i (color, t, continue) =
      let p = ro + rd ^* t
          ...
          newColor =
            (mix i' (vec3 (0.1, 0, 0.2)) (white ^* 0.9))
              * shade sdf p

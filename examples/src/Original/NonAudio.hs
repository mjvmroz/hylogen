...
module Original.NonAudio where
...
poop = vec4 (r, g, b, 1)
  where
    ...
    b =
      1 * val ** 2
        - (x_ uv * y_ uv + sin (time ** 0.1 + x_ audio))
        - 1
          * sin (x_ uv * 10 + time)
...
thang = 0.01 * vec4 (v, v, v, 1) + bb
  where
    ...
    circle x =
      10 - 10 * len (sin (uvN * 10 + 0.2 * x) + 0.5 * vec2 (cos $ time * 10, sin $ time * 10))
        & \x -> x / 5
...
drawingApp2 = vec4 (v, v, v, 1)
  where
    ...
    v =
      ...
      where
        invright =
          x_ (texture2D backBuffer (((uvN * res + 1) / res) * 0.5 + 0.5))
            & (\x -> (x * res + 1) / res)
